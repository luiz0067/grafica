...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT * FROM novidades  where (empresa=".$_SESSION["empresa"].") and (codigo=0".$_GET["codigo"].")";
...
if ($_POST){
	...
	)
	{
		...
		if (($_FILES["imagem"]["size"] < 5*1024*1024)){
			...
			else
			{
				...
				echo "<a href=\"../upload/imagens/novidades/p_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
			}
		}
		else
		{
	...
	if ($_POST ['acao']=='alterar'){
		...
		if ($imagem=="")
			$sql = "update novidades set  nome='".$_POST["nome"]."',empresa=".$_SESSION["empresa"]."  where (empresa=".$_SESSION["empresa"].") and (codigo=".$_POST["codigo"].");";
		else
			$sql = "update novidades set imagem='".$imagem."', nome='".$_POST["nome"]."',empresa=".$_SESSION["empresa"]."  where (empresa=".$_SESSION["empresa"].") and (codigo=".$_POST["codigo"].");";
		mysql_query($sql,$link);
		$sql = "commit";
		mysql_query($sql,$link);
	}
	else if( $_POST['acao']=='inserir'){
		$sql = "insert into novidades(imagem,nome,empresa)values('".$imagem."','".$_POST["nome"]."',".$_SESSION["empresa"].");";
		mysql_query($sql, $link);		
		$sql = "commit";
		mysql_query($sql,$link);
	}
		...
		$sql = "delete FROM novidades  where (empresa=".$_SESSION["empresa"].") and codigo=".$_POST["codigo"];
		mysql_query($sql, $link);
		//echo $sql;
		$sql = "commit";
		mysql_query($sql, $link);
...
?>
	...
	<form  method="post" enctype="multipart/form-data">
		<table >
			...
			<tr>
				<td>descrição:</td>
				<td><textarea name="nome"><?php if ($result!=null) echo $row["nome"]?></textarea></td>
			</tr>
			<tr>
				<td>imagem:</td>
			...
			<tr>
				<td colspan="2">
					...
					<a href="./cadastro_foto_novidades.php?novidades=<?php if ($result!=null) echo $row["codigo"]?>" target="_blank"><input type="button" value="inserir fotos"></a>
		...
		<?php
			...
			$sql="SELECT novidades.codigo,novidades.nome,novidades.descricao,novidades.imagem,novidades.empresa,novidades.link FROM novidades where(novidades.empresa=".$_SESSION["empresa"].");";
			$result = mysql_query($sql, $link);
			if (!$result) {
			...
			?>
				...
				<tr>
					<td colspan="2">
						<?php if($row['imagem']!=""){?>
						<a href="../upload/imagens/novidades/h_<?php echo $row['imagem'];?>" target="_blank">ampliar</a><br>
						<img width="100px" height="100PX" src="../upload/imagens/novidades/g_<?php echo $row['imagem'];?>">
						<?php 
						}else{?>&nbsp;<?php }?>
					</td>
				</tr>
